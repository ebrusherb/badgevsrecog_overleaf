...
\begin{document}
\noindent
\title{Large groups and learning costs promote the evolution of signal-based social assessment} 
\author{}
\date{} 
\maketitle
...
% STYLE NOTES:
% - how to treat rule-of-thumb / rule of thumb?
...
%%%%%%%%%%%%%%%%%%%%%%%
%Social information is important.
Animals in social groups benefit from having accurate information about their group mates \citep{Seyfarth:2010bh}. For example, an animal can make a better decision about who to fight with if it knows something about the dominance rank ~\citep{Waal:1986ys,Cowlishaw:1990vn,Bergman:2003qf,Seyfarth:2005ve,Flack:2006uq,Hobson:2015uq} or resource holding potential~\citep{Rhijn:1980uq,Freeman:1985kl,Dick:1990cr,Lemel:1993ve} of the other members of its group. A female can  make a better decision about who to mate with if she has some information about a male's likelihood of investing in parental care~\citep{Qvarnstrom:1997fk,Olsen:2010uq} or his health~\citep{Folstad:1992kx,Loyau:2005nx}. The information that animals have about each other can affect the type of social structure that emerges in their group \citep{Dugatkin:2004hz,Hobson:2015uq,Brush:2018ss}. In order to understand the evolution of social behavior, we need to understand the evolution of the strategies animals use to make assessments of each other.  

While much work has been done to understand how such signals can evolve, we know much less about the evolution of the assessment methods themselves \citep{sheehan2016evotradeoff}.


% SUMMARY OF SIGNAL-BASED ASSESSMENT
...
\subsection*{Learned rule of thumb}
...
The only parameters that can affect the innate rule-of-thumb are group size ($N$) and signal-quality correlation ($\rho$); because there is no learning involved, memory, updating, perceptive ability, and thus learning speed, are irrelevant. In the innate rule case, group size does not strongly affect learning accuracy (Figure \ref{parameters}A). [TKTK more about innate rule?]
...
\begin{figure}
%\includegraphics[width=6.85in]{figures/schematic_small_font.pdf}
\caption{\label{schematic}\sffamily\small\textbf{Animals can use signals to infer the quality values of conspecifics.} They can do so using either a linear rule, which can be either learned or genetic and which is determined by a slope and intercept, or categories, which can be coarse or fine. When there are as many categories as there are individuals in the group, categorical perception becomes individual recognition.}
\end{figure}
